...
if trigger_row:
    ...
    info(
        "Status:",
        f"{GREEN}active{RESET}" if trigger_row[3] else f"{YELLOW}paused{RESET}",
    )
...
api_call("GET", f"/v18.0/{POST_ID}/comments", "instagram_manage_comments")
print(
    f"    {DIM}params: fields=id,text,username,from,timestamp  limit=100  since=<last_polled_at>{RESET}"
)
print()
pause()
...
if leads:
    ...
    for lead in leads:
        ...
        print(
            f"  |{lid:>3} | @{(uname or ''):<18}| {(status or ''):<9} | {kw:<8} | {dm_flag}    |"
        )
    print(f"  +----+--------------------+-----------+----------+--------+")
    print()
pause(2)
...
print(
    f"    {DIM}params: fields=id,text,username,from,timestamp  limit=100  since=<unix_ts>{RESET}"
)
print(f"""
