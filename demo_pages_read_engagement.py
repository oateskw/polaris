"""
Screencast demo for a new Meta application review -- pages_read_engagement permission.
...
api_call("GET", f"/v18.0/{ig_user_id}/media", "pages_read_engagement")
print(
    f"    {DIM}fields=id,caption,media_type,like_count,comments_count,timestamp{RESET}"
)
print()
pause()
...
if media_list:
    ...
    for m in media_list:
        ...
        print(
            f"  | {mid:<19} | {mtype:<5} | {ts:<9} | {str(likes):<4} | {str(comms):<8} |"
        )
    print(f"  +---------------------+-------+-----------+------+----------+")
else:
...
""")
api_call(
    "GET", f"/v18.0/{ig_user_id}/media", "fields include like_count, comments_count"
)
print(f"""
