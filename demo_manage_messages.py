...
cur = conn.cursor()
cur.execute(
    "SELECT access_token, instagram_user_id, username FROM instagram_accounts WHERE id=1"
)
...
for c in comments:
    ...
    print(
        f"    [{c.get('timestamp','')[:10]}]  \"{c.get('text','')}\"  @{c.get('username','unknown')}{flag}"
    )

if not comments:
    print(
        f"    {DIM}(No comments yet — keyword match shown in simulation below){RESET}"
    )
...
api_call(
    "POST", f"/v18.0/<comment_id>/private_replies", "instagram_business_manage_messages"
)
print()
...
api_call("GET", "/v18.0/me/conversations", "instagram_business_manage_messages")
print(
    f"    {DIM}params: platform=instagram, fields=messages{{id,message,from,created_time}}{RESET}"
)
pause()

FOLLOWER_REPLY = (
    "I run a landscaping company and I'm drowning in quote requests and scheduling"
)
...
api_call("POST", "/v18.0/me/messages", "instagram_business_manage_messages")
print(
    f'    {DIM}payload: recipient={{id: <instagram_scoped_user_id>}}, message={{text: "..."}}{RESET}'
)
pause(2)
...
if not existing:
    full_history = json.dumps(
        [
            ...
            {
                "role": "user",
                "message": FOLLOWER_REPLY,
                "timestamp": "2026-02-25T15:04:00",
            },
            {
                "role": "assistant",
                "message": ai_reply,
                "timestamp": "2026-02-25T15:04:45",
            },
        ]
    )
    cur.execute(
...
print(
    f"  Run {BOLD}polaris leads show {lead_id}{RESET} to see the full conversation:\n"
)
pause()
