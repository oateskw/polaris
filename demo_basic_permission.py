"""
Screencast demo for a new Meta application review — instagram_business_basic permission.
...
print(
    f"  {CYAN}Waiting for OAuth callback on http://localhost:8000/callback ...{RESET}"
)
...
for p, status in sorted(perms.items()):
    marker = (
        f"{GREEN}granted{RESET}" if status == "granted" else f"{DIM}{status}{RESET}"
    )
