"""
Screencast demo for a new Meta application review -- instagram_content_publish permission.
...
api_call("POST", f"/v18.0/{ig_user_id}/media", "image_url=<cdn_url>  caption=<text>")
api_call("GET", "/v18.0/{container_id}", "fields=status_code,status")
api_call("POST", f"/v18.0/{ig_user_id}/media_publish", "creation_id={container_id}")
pause()
...
api_call(
    "POST", f"/v18.0/{ig_user_id}/media_publish", "creation_id={carousel_container_id}"
)
pause()

print(f"\n  {BOLD}D) Instagram Story (Image){RESET}\n")
api_call(
    "POST", f"/v18.0/{ig_user_id}/media", "image_url=<cdn_url>  media_type=STORIES"
)
api_call("GET", "/v18.0/{container_id}", "fields=status_code,status")
api_call("POST", f"/v18.0/{ig_user_id}/media_publish", "creation_id={container_id}")
pause()
...
api_call(
    "POST", f"/v18.0/{ig_user_id}/media", "video_url=<cdn_url>  media_type=STORIES"
)
api_call(
    "GET", "/v18.0/{container_id}", "fields=status_code,status  (poll until FINISHED)"
)
api_call("POST", f"/v18.0/{ig_user_id}/media_publish", "creation_id={container_id}")
pause(2)
...
for row in recent_content:
    ...
    color = (
        GREEN if status == "PUBLISHED" else (YELLOW if status == "SCHEDULED" else DIM)
    )
