...
api_call("GET", "/v18.0/me/conversations", "instagram_manage_messages")
print(
...
if "data" in convos and convos["data"]:
    ...
    for c in convos["data"][:3]:
        ...
        if msgs:
            last = msgs[0]
            frm = last.get("from", {}).get("username") or last.get("from", {}).get(
                "id", "--"
            )
...
api_call("POST", "/v18.0/me/messages", "instagram_manage_messages")
print(f"""
...
""")
api_call("GET", "/v18.0/me/conversations", "platform=instagram  fields=messages{...}")

print(f"""
  {BOLD}2. Send AI-generated follow-up DM to a lead:{RESET}
""")
api_call("POST", "/v18.0/me/messages", "recipient={{id}} message={{text}}")
