...
cur = conn.cursor()
cur.execute(
    "SELECT access_token, instagram_user_id, username, followers_count FROM instagram_accounts WHERE id=1"
)
...
api_call("GET", "/v18.0/{media_id}/insights", "instagram_manage_insights")
...
api_call("GET", f"/v18.0/{ig_user_id}/insights", "instagram_manage_insights")
...
if stored_metrics:
    ...
    print(
        f"  {DIM}(No metrics stored yet -- run: polaris analytics fetch --account 1){RESET}"
    )
...
""")
api_call("GET", "/v18.0/{media_id}/insights", "metric=reach,saved,shares")

print(f"""
  {BOLD}2. Account-level insights:{RESET}
""")
api_call(
    "GET", f"/v18.0/{ig_user_id}/insights", "metric=reach,follower_count  period=day"
)
